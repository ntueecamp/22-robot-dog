...
bool AudioProcessor::get_spectrogram(int16_t* input_audio, float* output_spectrogram)
{
    ...
    for (int i = 0; i < m_audio_length; i++)
    {
        float value = fabsf(input_audio[i] - mean);
    ...
    for (int window_start = 0; window_start < 16000 - m_window_size; window_start += m_step_size)
    {
        // read samples from the input_audio into the fft input normalising them by subtracting the mean and dividing by the absolute max
        for (int i = 0; i < m_window_size; i++)
        {
            m_fft_input[i] = (input_audio[i] - mean) / max;
        }
